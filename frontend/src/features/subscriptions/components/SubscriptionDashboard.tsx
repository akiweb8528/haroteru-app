...
export function SubscriptionDashboard({ isGuest = false }: Props) {
  ...
  const guestSyncCopy = taste === 'simple'
    ? '現在のデータはこの端末に保存されています。ブラウザの設定や利用状況によって消えることがあるため、残しておきたい場合は Google で同期すると別の端末でも同じ一覧を利用できます。'
    : '今のデータはこの端末に入っとるで。ただ、ブラウザの設定や使い方しだいで消えることもあるんや。残しときたいなら、Googleで同期したってや。別の端末でも同じ一覧が見られるで。';
