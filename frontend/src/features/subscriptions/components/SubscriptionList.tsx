...
export function SubscriptionList({ subscriptions, isLoading, error, onUpdate, onDelete }: Props) {
  ...
  if (error) {
    return <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-8 text-center text-sm text-red-700 dark:border-red-900/30 dark:bg-red-900/10 dark:text-red-300">一覧の読み込みに失敗しました。</div>;
  }

  if (subscriptions.length === 0) {
    return <div className="rounded-2xl border-2 border-dashed border-gray-200 px-6 py-12 text-center text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">まだサブスクがありません。まずは1件登録して、今の支出を見える化しましょう。</div>;
  }
