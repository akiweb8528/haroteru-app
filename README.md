...
- `haroteru-frontend-staging`
- `NEXTAUTH_URL`: frontend の公開 URL。例: `https://haroteru-frontend-staging.onrender.com`
- `GOOGLE_CLIENT_ID`
- `GOOGLE_CLIENT_SECRET`
- `NEXT_PUBLIC_GA_MEASUREMENT_ID`: Google Analytics の Measurement ID。例: `G-0T26JZ2M17`
- `NEXT_PUBLIC_APP_ENV`: Google Analytics などで環境を識別する公開値。staging は `staging`

サービス間通信は Blueprint で連携されます。

- frontend の `BACKEND_URL` は backend の `PUBLIC_API_URL` を参照
- frontend の `NEXT_PUBLIC_API_URL` は backend の `PUBLIC_API_URL` を参照
- backend の `FRONTEND_URL` は frontend の `NEXTAUTH_URL` を参照
