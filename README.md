...
このリポジトリは、アプリ本体だけでなく AI 開発フローもポートフォリオとして見せる前提で整理しています。
...
- `.serena/project.yml`: Serena のプロジェクト設定。Go / TypeScript を対象にしたローカルエージェント運用の前提だけを公開用に残しています

汎用的な外部 skill 群は含めず、このプロジェクトを理解して改善するために必要な最小セットだけを残しています。
