...
4. Dev Container 作成後に次を実行する

```bash
docker compose up
```

`.devcontainer/devcontainer.json` で Node.js 20 / Go 1.26 / Docker CLI を揃えており、`postCreateCommand` で `frontend` の `npm install` と `backend` の `go mod download` を自動実行します。
